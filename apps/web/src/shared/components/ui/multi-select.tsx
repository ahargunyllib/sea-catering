...
const MultipleSelector = ({
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: `handleClickOutside` is a stable function that does not change on every render.
	useEffect(() => {
		if (open) {
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: `arrayDefaultOptions` and `arrayOptions` are the initial options.
	useEffect(() => {
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: `debouncedSearchTerm` is used to trigger search.
	useEffect(() => {
		/** sync search */
	...
	// biome-ignore lint/nursery/noNestedComponentDefinitions: This is a common pattern in React to avoid re-creating components on every render.
	const CreatableItem = () => {
	...
	return (
		...
		>
			{/** biome-ignore lint/a11y/noStaticElementInteractions: This is a workaround for cmdk to avoid the first item selected. */}
			{/** biome-ignore lint/a11y/useKeyWithClickEvents: This is a workaround for cmdk to avoid the first item selected. */}
			<div
				className={cn(
